...
class HelperDetailsComponent extends StatelessWidget implements HelperDetailsInterface {
  ...
  HelperDetailsComponent({Key key, @required this.helper}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return _mvvmPageBuilder.build(
      context: context,
      presenterBuilder: (context) => HelperDetailsPresenter(HelperDetailsModel(), this),
      builder: (context, presenter, model) => SafeArea(
        child: Scaffold(
          appBar: _buildAppBar(context.buildContext),
  ...
  AppBar _buildAppBar(BuildContext context) => AppBar(
        ...
        actions: [Icon(Icons.delete),Divider(endIndent: 14,)],
      );

  _buildBody(BuildContext context) => Column(
        children: [
          _buildLine('Available on version', '${this.helper.versionMin} - ${this.helper.versionMax}',context),
          _buildLine('Trigger mode', getHelperTriggerTypeDescription(this.helper.triggerType),context)
        ],
      );

  _buildLine(String label, String text,BuildContext context) {
