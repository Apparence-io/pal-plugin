...
void main() {
  ...
  _testCreateHelper(dynamic args, HelperEntity expectedHelperResult, Function creationCall) async {
    ...
    when(() => httpClientMock.post(Uri.parse('pal-business/editor/groups/${args.helperGroup.id}/helpers'), body: any(named: "body")))
      .thenAnswer((_) => Future.value(Response(expectedHelperResultJson, 200)));
    await creationCall();
    // group exists here and should not been called
    verifyNever(() => httpClientMock.post(Uri.parse('pal-business/editor/pages/${existingPage.id}/groups'), body: captureAny(named: "body")));
  ...
  _testCreateHelperWithGroupAndVersionAndPage(dynamic args, HelperEntity expectedHelperRequest, Function creationCall) async {
    ...
    when(() => httpClientMock.post(Uri.parse('pal-business/editor/pages/${existingPage.id}/groups'), body: any(named: 'body')))
    ...
    when(() => httpClientMock.post(Uri.parse('pal-business/editor/groups/89032803JS/helpers'), body: any(named: "body")))
      .thenAnswer((_) => Future.value(Response(expectedHelperResultJson, 200)));
    await creationCall();
    ...
    var capturedGroupCreationCall =  verify(() => httpClientMock.post(Uri.parse('pal-business/editor/pages/${existingPage.id}/groups'), body: captureAny(named: "body"))).captured;
    expect(capturedGroupCreationCall[0], equals(groupCreationReqJson));
    var capturedHelperCreationCall =  verify(() => httpClientMock.post(Uri.parse('pal-business/editor/groups/89032803JS/helpers'), body: captureAny(named: "body"))).captured;
  ...
  _testUpdateHelper(dynamic args, HelperEntity expectedHelperResult, Function updateCall) async {
    ...
    when(() => httpClientMock.put(Uri.parse('pal-business/editor/helpers/${args.config.id}'), body: any(named: "body")))
      .thenAnswer((_) => Future.value(Response(expectedHelperResultJson, 200)));
    await updateCall();
    // group exists here and should not been called
    ...
    verifyNever(() => httpClientMock.post(
      Uri.parse('pal-business/editor/pages/${existingPage.id}/groups'), 
      body: captureAny(named: "body"))
    );
  }
