...
void main() {
  group('EditorHelperService', () {
    ...
    test('[SimpleHelper] create an helper should call editor helper API and return entity', () async {
      ...
      HelperEntity myHelper = HelperEntity(
        ...
        helperTexts: [
          HelperTextEntity(
            fontColor: "#CCC", fontWeight: "w100", fontSize: 21,
      ...
      HelperEntity resHelper = HelperEntity.copy(myHelper)
        ..id = "820938203";
      ...
      var resultEntity = await editorHelperService.createSimpleHelper(pageId, args);
    ...
    test('[FullscreenHelper] create an helper should call editor helper API and return entity', () async {
      ...
      HelperEntity myHelper = HelperEntity(
        ...
        helperImages: [HelperImageEntity(id: args.topImageId, key: FullscreenHelperKeys.IMAGE_KEY)],
        helperBoxes: [HelperBoxEntity(key: FullscreenHelperKeys.BACKGROUND_KEY, color: args.backgroundColor)]
      );
      // what http will result
      HelperEntity resHelper = HelperEntity.copy(myHelper)
        ..id = "820938203";
      ...
      when(httpClientMock.post('editor/pages/$pageId/helpers', body: reqHelperJson))
        .thenAnswer((_) => Future.value(Response(resHelperJson, 200)));
      // call the service part
      var resultEntity = await editorHelperService.createFullScreenHelper(pageId, args);
      expect(resultEntity, isNotNull, reason: "The service didn't create entity properly");
    });
    ...
    test('[UpdateHelper] create an updateHelper should call editor helper API and return entity', () async {
      ...
      when(httpClientMock.post('editor/pages/$pageId/helpers', body: reqHelperJson))
        .thenAnswer((_) => Future.value(Response(resHelperJson, 200)));
      // call the service part
      var resultEntity = await editorHelperService.createUpdateHelper(pageId, args);
      expect(resultEntity, isNotNull, reason: "The service didn't create entity properly");
    });
  });
}
